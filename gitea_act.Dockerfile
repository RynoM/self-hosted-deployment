...
RUN apt-get install
RUN apt-get install -yq curl nodejs git
...
CMD ["sh", "-c", "./act_runner register --instance $INSTANCE_URL --token $RUNNER_TOKEN --no-interactive --labels self-hosted;./act_runner daemon"]